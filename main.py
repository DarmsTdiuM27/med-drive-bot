...
class ReplitPing(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        # Allow root and health endpoint (for monitors like UptimeRobot + Replit health checks)
        allowed = {"/", HEALTH_PATH, "/health"}
    ...
    for mnum, module_id, _module_name in modules:
        ...
        if new_items:
            ...
            for it in new_items:
                ...
                actions.append((CHANNEL_ID, msg))
                for uid in users_for_module(mnum):
                    actions.append((uid, msg))
...
async def monitor_job(context: ContextTypes.DEFAULT_TYPE):
    """
    Periodic job that checks Drive updates and sends notifications.
    """
    actions = await asyncio.to_thread(_collect_notifications)

    # Send sequentially to avoid hitting Telegram flood limits
    for chat_id, msg in actions:
        try:
...
async def on_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("NOTIFY:CHOOSE:"):
        ...
        if len(parts) < 4:
            return
        _, _, mnum, folder_id = parts

        user_entry = STATE.setdefault("users", {}).setdefault(str(uid), {"module": None, "module_folder_id": None, "dm_enabled": True})
        user_entry["module"] = str(mnum)
...
def main():
    ...
    # Schedule periodic monitor job (async, non-blocking)
    app.job_queue.run_repeating(monitor_job, interval=MONITOR_INTERVAL_SECONDS, first=10)

    # Run polling (retry on Conflict if another instance is still running)
    while True:
        try:
